...
while wb_robot_step(TIME_STEP) ~= -1
  ...
  [targetIndex, targetPoint] = purePursuit(step, position_data, trajectory, targetIndex, L);
  
  angle = wb_compass_get_values(compass);
  theta_ = pi/2-atan2(angle(2), angle(1));
  
  thetak = atan2(targetPoint(2) - position_data(step,2), targetPoint(1) - position_data(step,1));
 
  
  error_speed=sqrt((targetPoint(1)-position_data(step,1))^2+(targetPoint(1)-position_data(step,2))^2);
...
function [target_idx, target_point] = purePursuit(step, pose, trajectory, target_idx, L)
  
  while target_idx < size(trajectory,1)
    distance = calculateDistance(trajectory(target_idx,:), pose(step,:));
    ...
    % If all waypoints reached, return the last waypoint

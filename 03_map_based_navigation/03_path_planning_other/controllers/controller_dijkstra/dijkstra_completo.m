...
[end_node, shortest_distances, path, error, path_cost] = dijkstra_with_path(q, start_node, end_node);
time_elapsed = toc;
...
save('path.mat', "path_GPS");

% Plot the GPS path
clf(figure(2));
figure(2);
plot(path_GPS(:, 1), path_GPS(:, 2), '-o', 'LineWidth', 2, 'MarkerSize', 8);
xlabel('X axis (m)');
ylabel('Y axis (m)');
title('Planned Path');
grid on;
...
function [end_node, shortest_distances, path, error, path_cost] = dijkstra_with_path(adjacency_matrix, start_node, end_node)
    ...
    while any(~visited)
        ...
        [~, current_node] = min(shortest_distances(unvisited_nodes));
